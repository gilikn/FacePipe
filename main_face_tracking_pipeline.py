...
from facepipe.config.pre_processing_config import CROP_DIRECTORY, AUDIO_DIRECTORY
from facepipe.core.run_face_detection import run_face_detection_pipeline, add_audio_pipeline

flags.DEFINE_string('base_directory_path', None, 'The path to the directory where original and results directories are held.')
...
flags.DEFINE_string('mean_face_path', None, "reference mean face (download from: "
...
def main(argv):
    ...
    assert FLAGS.mean_face_path is not None, "Please enter the path to the mean_face file!"
